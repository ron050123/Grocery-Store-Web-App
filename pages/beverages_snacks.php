...
// fetch all products in the "Beverages & Snacks" category
$query = "SELECT * FROM products WHERE product_name IN ('Earl Grey Tea Bags', 'Instant Coffee', 'Chocolate Bar')";

// selected category
if ($category) {
    if ($category == 'tea_coffee') {
        $query .= " AND product_name IN ('Earl Grey Tea Bags', 'Instant Coffee')";
    } elseif ($category == 'snacks') {
        $query .= " AND product_name IN ('Chocolate Bar')";
    }
}
